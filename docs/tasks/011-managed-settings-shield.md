...
## In scope
- Concrete `ScreenTimeShieldService` in `Transition/ScreenTime/Shielding/`.
- Apply and remove shields for the persisted `FamilyActivitySelection` (apps, categories, web domains).
- A named/dedicated ManagedSettings store owned by Transition.
...
§15 and §10 rule 5: **remove only Transition-managed shielding.** Never indiscriminately clear all settings.
...
- Use a **named** `ManagedSettingsStore` dedicated to Transition. Clearing that store must never touch settings written by Apple's own Screen Time or by another parental-control app on the same device.
...
- [ ] The store is named and owned by Transition; no global clear exists anywhere in the codebase.
